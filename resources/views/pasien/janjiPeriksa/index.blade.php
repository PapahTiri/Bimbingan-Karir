                ...
                <div class="max-w-xl">
                    <section>
                        ...
                        <form class="mt-6 space-y-6" action="{{ route('pasien.janjiPeriksa.store') }}" method="POST">
                            ...
                            {{-- Pilih Dokter --}}
                            <div>
                                <label for="dokterSelect" class="block text-sm font-medium text-gray-700 mb-1">Dokter</label>
                                    ...
                                    @foreach ($dokters as $dokter)
                                        @foreach ($dokter->jadwalPeriksas as $jadwal)
                                            <option value="{{ $dokter->id }}">
                                                {{ $dokter->nama }} - Spesialis {{ $dokter->poli->nama_poli }} | {{ $dokter->poli->deskripsi }} | {{ $jadwal->hari }},
